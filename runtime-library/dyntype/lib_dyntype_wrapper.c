                                 ...
                                 dyn_value_t *args)
{
    ...
    bh_memcpy_s(argv + occupied_slots, sizeof(argv) - occupied_slots,
                &context.gc_obj, sizeof(wasm_anyref_obj_t));
    ...
    if (cb_count) {
        ...
        res = (wasm_value_t)(uintptr_t)&argv[0];
        if (cb_ret_ref_type.value_type == VALUE_TYPE_F64) {
            return dyntype_new_number(dyntype_get_context(), res.f64);
        } else if (cb_ret_ref_type.value_type == VALUE_TYPE_I32) {
            return dyntype_new_boolean(dyntype_get_context(), res.i32);
        } else if (is_ts_string_type(module, wasm_obj_get_defined_type(
                                               (wasm_obj_t)res.gc_obj))) {
            wasm_struct_obj_get_field((wasm_struct_obj_t)res.gc_obj, 1, false,
                                      &field1);
            wasm_array_obj_t a_ref = (wasm_array_obj_t)(field1.gc_obj);

            const char *str =
                (const char *)wasm_array_obj_first_elem_addr(a_ref);
            len = wasm_array_obj_length(a_ref);
            return dyntype_new_string_with_length(dyntype_get_context(), str, len);
        } else {
            ptr = res.gc_obj;
            RETURN_BOX_ANYREF(ptr);
        }
    }
...
static void *
box_value_to_dyntype(wasm_exec_env_t exec_env, dyn_ctx_t ctx, void *value,
...
unbox_value_from_dyntype(wasm_exec_env_t exec_env, dyn_ctx_t ctx, void *obj,
                         wasm_ref_type_t type, void *unboxed_value,
                         uint32 slot_count)
{
    uint32 table_idx = 0;
    wasm_defined_type_t ret_defined_type = { 0 };
                    ...
                    wasm_struct_obj_t args_array)
{
    ...
    if (!wasm_runtime_call_func_ref(exec_env, func_obj, bsize, argv)) {
        wasm_runtime_set_exception(wasm_runtime_get_module_inst(exec_env),
